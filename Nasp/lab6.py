...
class SkipList:
    ...
    def __init__(self, n: int = 10000, p: float = 0.5):
        ...
        self.p = p
        self.n = n
        self.max_level = ceil(log(self.n, 1 / self.p))
    ...
    def _createHistogram(self) -> List:
        ...
        # Calculate cumulative histogram directly
        for i in range(1, self.max_level + 1):
            H[i] = ceil(self.n * (1 - self.p ** (i)))


        # Ensure the total aligns with `n`
        H[self.max_level] = self.n
    ...
    def sampleLevel(self) -> int:
        ...
        i = 1
        while i <= self.max_level and ctoss > self.histogram[i]:
            i += 1
        return i
    ...
    def insert(self, value: int) -> None:
        ...
        """
        level = min(self.sampleLevel(), self.max_level)

        pred_node = self.head
        update = [None] * level

        for lvl in range(level - 1, -1, -1):
            while pred_node.pointers[lvl] is not None and pred_node.pointers[lvl].value < value:
                pred_node = pred_node.pointers[lvl]
            update[lvl] = pred_node

        if pred_node.pointers[0] is not None and pred_node.pointers[0].value == value:
            return
        ...
        for lvl in range(level):
            new_node.pointers[lvl] = update[lvl].pointers[lvl]
            update[lvl].pointers[lvl] = new_node
    ...
    def __str__(self) -> str:
        ...
        while curr_node is not None:
            result += f"(Node {curr_node.value},level={len(curr_node.pointers)}),"
